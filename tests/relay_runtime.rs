...
// ==========================================================================
// STEP-12.16 Phase 7a — Logging-coverage remediation
//
// Tests to ensure the daemon properly logs lifecycle events around logout,
// leave, connect retry errors, heartbeat suspension edges, and startup
// validation.
// ==========================================================================

/// `logout_success_emits_runtime_logout_succeeded`
#[tokio::test]
#[tracing_test::traced_test]
async fn logout_success_emits_runtime_logout_succeeded() {
    use wiremock::{MockServer, Mock, ResponseTemplate};
    use wiremock::matchers::{method, path};
    use ranchero::daemon::relay::DefaultAuthLogin;
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path(zwift_api::TOKEN_PATH)).respond_with(ResponseTemplate::new(200).set_body_string("{\"access_token\": \"ATOK\", \"refresh_token\": \"RTOK\", \"expires_in\": 600, \"refresh_expires_in\": 2400, \"token_type\": \"Bearer\"}")).mount(&server).await;
    Mock::given(method("GET")).and(path("/api/profiles/me")).respond_with(ResponseTemplate::new(200).set_body_string("{\"id\": 12345}")).mount(&server).await;
    Mock::given(method("GET")).and(path("/relay/worlds/1/players/12345")).respond_with(ResponseTemplate::new(200).set_body_string("{\"world\": 1}")).mount(&server).await;
    Mock::given(method("POST"))
        .and(path("/relay/worlds/1/leave"))
        .respond_with(ResponseTemplate::new(200))
        .mount(&server)
        .await;
    Mock::given(method("POST")).and(path("/api/users/logout")).respond_with(ResponseTemplate::new(200)).mount(&server).await;
    let mut cfg = make_config("rider@example.com", "secret");
    cfg.zwift_endpoints.auth_base = server.uri();
    cfg.zwift_endpoints.api_base = server.uri();
    ...
    let default_auth = DefaultAuthLogin::new(auth.clone());
    let mut runtime = RelayRuntime::start_with_all_deps(&cfg, None, default_auth, StubSupervisorFactory::new(fixture_session()), StubTcpFactory::new(), NoopUdpFactory).await.expect("start");
    runtime.shutdown_auth = Some(auth);
    runtime.shutdown();
    let _ = runtime.join().await;
    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    assert!(tracing_test::internal::logs_with_scope_contain("ranchero", "relay.runtime.logout_succeeded"));
}

/// `leave_success_emits_runtime_leave_succeeded`
#[tokio::test]
#[tracing_test::traced_test]
async fn leave_success_emits_runtime_leave_succeeded() {
    use wiremock::{MockServer, Mock, ResponseTemplate};
    use wiremock::matchers::{method, path};
    use ranchero::daemon::relay::DefaultAuthLogin;
    let server = MockServer::start().await;
    ...
    Mock::given(method("GET")).and(path("/api/profiles/me")).respond_with(ResponseTemplate::new(200).set_body_string("{\"id\": 12345}")).mount(&server).await;
    Mock::given(method("GET")).and(path("/relay/worlds/1/players/12345")).respond_with(ResponseTemplate::new(200).set_body_string("{\"world\": 1}")).mount(&server).await;
    Mock::given(method("POST")).and(path("/relay/worlds/1/leave")).respond_with(ResponseTemplate::new(200)).mount(&server).await;
    let mut cfg = make_config("rider@example.com", "secret");
    cfg.zwift_endpoints.auth_base = server.uri();
    cfg.zwift_endpoints.api_base = server.uri();
    let auth_config = zwift_api::Config {
    ...
    let _ = runtime.join().await;
    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    assert!(tracing_test::internal::logs_with_scope_contain("ranchero", "relay.runtime.leave_succeeded"));
}

/// `connect_retry_logs_underlying_error_cause`
#[tokio::test(start_paused = true)]
#[tracing_test::traced_test]
async fn connect_retry_logs_underlying_error_cause() {
    let cfg = make_config("rider@example.com", "secret");
    let _ = RelayRuntime::start_with_all_deps(&cfg, None, StubAuth, StubSupervisorFactory::new(fixture_session()), TransientTcpFactory::new(), NoopUdpFactory).await;
    tokio::time::sleep(std::time::Duration::from_millis(1500)).await;
    assert!(tracing_test::internal::logs_with_scope_contain("ranchero", "relay.runtime.connect_retry"));
    assert!(tracing_test::internal::logs_with_scope_contain("ranchero", "error="));
}

/// `heartbeat_tick_suspended_fires_only_on_edge`
#[tokio::test(start_paused = true)]
#[tracing_test::traced_test]
async fn heartbeat_tick_suspended_fires_only_on_edge() {
    use ranchero::daemon::relay::HeartbeatScheduler;
    use ranchero::daemon::relay::HeartbeatSink;
    use std::sync::Arc;
    use std::sync::atomic::{AtomicBool, Ordering};
    use zwift_relay::WorldTimer;
    
    struct DummySink;
    impl HeartbeatSink for DummySink {
        async fn send(&self, _state: zwift_proto::PlayerState) -> std::io::Result<()> {
            Ok(())
        }
    }
    ...
    // Advance 5 ticks while suspended
    tokio::time::sleep(std::time::Duration::from_millis(550)).await;
    
    // Clear suspended, advance 2 ticks
    ...
    // Currently it fires every tick, so there will be many.
    // We assert the new events exist (which will fail until 7b).
    assert!(tracing_test::internal::logs_with_scope_contain("ranchero", "relay.heartbeat.suspended"));
    assert!(tracing_test::internal::logs_with_scope_contain("ranchero", "relay.heartbeat.resumed"));
}

/// `validation_failure_emits_structured_trace`
#[tokio::test]
#[tracing_test::traced_test]
async fn validation_failure_emits_structured_trace() {
    // Call validate_startup with missing watched_athlete_id
    let mut cfg = make_config("rider@example.com", "secret");
    cfg.watched_athlete_id = None;
    
    // validate_startup is in daemon::validate but exposed through daemon module maybe?
    let result = ranchero::daemon::validate::validate_startup(&cfg, None);
    assert!(result.is_err());
    
    // The test specifies we call validate_startup and assert the daemon log
    // contains a ranchero.startup.validation_failed event.
    assert!(tracing_test::internal::logs_with_scope_contain("ranchero", "ranchero.startup.validation_failed"));
    assert!(tracing_test::internal::logs_with_scope_contain("ranchero", "missing="));
}
