...
- `ranchero start --capture <path>` opens a writer; capture is off
  when the flag is absent (zero overhead in that path — `Option::None`
  branch in the channel hot path).
- A single `Capture` tap covers both UDP (STEP 10) and TCP
  (STEP 11) channels. Tap sits on the recv path **after**
  AES-GCM tag verification and **before** prost decode, and on the
  send path **after** prost encode and **after** envelope stripping
  (so what's stored is exactly the bytes that go into / come out of
  `prost::decode`).
- `ranchero replay <path>` reads a capture and prints a summary.
  Programmatic `CaptureReader` (an `Iterator`) is the surface STEPS
  18 / 19 consume for parity tests.

## Scope

**In scope**:

- Append-only file format: 10-byte header (magic + version),
  variable-length records (timestamp + direction + transport +
  flags + length + payload). Concrete byte spec below.
- `CaptureWriter` — opens the file, owns a background writer task,
  exposes a non-blocking sync `record(&self, …)` for the channel
  hot path, exposes `dropped_count()`, and a `flush_and_close()`
...
- `UdpChannelConfig::capture` and `TcpChannelConfig::capture` —
  ...
  check that LLVM should fold into a no-op branch when the field
...
|---|---|
| `--capture-raw` (ciphertext + IV + AAD + AES key) | Stub already deferred. Useful only for STEP 08-style codec replay; we have hand-built test vectors there already. Not blocking STEP 18/19. |
| Capture rotation / size limits | Single file per `start` invocation is enough for the fixture use case. |
| `CaptureReader` doing full pipeline replay (decode → stats → WS broadcast) | STEP 18+ owns the pipeline; this step just provides the bytes. |
| Multi-channel disambiguation (per-UDP-pool channel id) | Out for v1. Single UDP channel is the steady-state today. If a multi-UDP capture is ever needed, the `flags` byte has 7 reserved bits and the format version can bump. |
| `fsync` per record | Per-close `fsync` (via `flush_and_close`) is enough for fixture use; per-record would devastate throughput. |
| Compression | Plaintext proto bytes compress well, but adds a dep + complexity. Add only if file sizes become a problem in practice. |

## Crate layout

`zwift-relay` already owns the channels that need the tap. Adding
the capture there keeps coupling minimal — no cross-crate dep edge.
...
If `capture.rs` grows past ~500 lines or develops independent
sub-concerns, fold into `capture/{mod, format, writer, reader}.rs`.
Plan starts flat.
...
added (the `net` feature is already in for STEP 10). Verify against
what `tokio::fs::File` requires when we get there. No new direct
deps expected.
...
offset 11..15 : len              u32 LE  — payload byte count, capped at 65 535 (`u16::MAX` — TCP frame ceiling)
offset 15..   : payload          [u8; len] — the bytes that go into / come out of prost::decode
```
...
`payload` to `decode_to_vec`-style methods) at the cost of one tiny
strip on the outbound tap.

### Why little-endian, not big-endian

Wire formats use BE. Captures are local-machine artifacts written
and read on the same architecture (almost always x86-64 / arm64,
...
The `record()` API is *deliberately sync*. Channels call it from
inside their hot loops where `await` would either complicate
control flow (yet another `select!` arm) or risk introducing
ordering questions. `try_send` on an `mpsc::Sender` is the right
primitive.
...
The reader is **sync**, not async. Replay use cases iterate the
file at their own pace; async machinery would only complicate the
common case (read once, iterate, decode). If batch-replay later
wants concurrent file reads + decode, wrap the iterator in a
`spawn_blocking`.
...
`Default` impls set `capture: None`. Production callers (STEP 12
supervisor) wire it in when the user passed `--capture <path>`;
tests construct with `None` and aren't affected.
...
- **Inbound** (recv loop): immediately after `process_inbound_packet`
  ...
  the tap actually fires inside `process_inbound_packet` after
...
- **Inbound**: same shape as UDP — `process_inbound` returns the
  decrypted `Vec<u8>`; loop decodes + records.
...
User-visible (not xtask) since the stub asked for the option to
explore captures interactively.

## Tests-first plan

All tests in `crates/zwift-relay/tests/capture.rs`. Channel-tap
tests live in the existing `tests/udp.rs` and `tests/tcp.rs`
(extend, don't duplicate the mock-transport setup).
...
| Test | Asserts |
|---|---|
| `writer_drops_when_channel_full` | Open a writer with a deliberately tiny channel (e.g. capacity 1, exposed via a test-only constructor `CaptureWriter::open_with_capacity`); push 1 000 records as fast as possible; assert `dropped_count() > 0` and the writer task didn't panic. |
| `writer_record_is_non_blocking` | Spawn a slow writer (block file writes via a stub or `tokio::time::sleep` injected); assert `record()` returns within 1 ms even when many records are pending. |
...
is the contract that matters. Add a bench later if profiling shows
the null check itself is a problem.)

### Channel tap (in `tests/udp.rs` / `tests/tcp.rs`)

| Test | Asserts |
|---|---|
| `udp_channel_with_capture_records_inbound_packets` | Set up a UdpChannel with `capture: Some(writer)`; drive convergence + push 3 inbound replies; close the writer; reader sees 3 inbound records (plus possibly hello-loop replies — assert at least 3). |
| `udp_channel_with_capture_records_outbound_player_state` | After convergence, `send_player_state` 5 times; reader sees outbound records with the proto-only payload (no `[1]` envelope byte). |
| `tcp_channel_with_capture_records_inbound_packets` | Similar for TCP recv side. |
...
1. **Timestamp source.** Plan uses Unix-epoch nanoseconds (absolute,
   reproducible across captures, easy to compare). Alternatives
   considered:
   ...
   - Monotonic since capture-start: ordering-correct, smaller, but
     can't be aligned across captures. Rejected.
...
2. **Channel-id disambiguation in `flags`.** Bit 0 of `flags` is
   ...
   captures, two paths:
   - Use bits 1–4 as a 4-bit channel id (max 16 channels per
     transport per file).
   - Bump format version, add a `channel_id: u8` field to the
     record header.

   No decision needed today; v1 single-UDP-channel is the steady
   state.

3. **Drop policy: oldest vs. newest on channel-full.** Plan: drop
   newest (the one being recorded). Rationale: keeps the writer
   from having to coordinate with already-queued records; preserves
   ordering for whatever does get written. Sauce-style "drop
   oldest" would let the most recent state survive at the cost of
   gaps in the timeline that confuse parity tests. Newest-drop +
   counter is honest about what was lost.

4. **`flush_and_close` ordering at shutdown.** Plan: supervisor
   (STEP 12) calls it after channels have shut down. Open question
   for implementation: does the supervisor wait for in-flight
   record sends from the channel hot path to complete before
   closing the writer? Tests should pin this — closing while a
   record is mid-flight should not lose it (or should clearly drop
   + count).
...
6. **Bytes vs. Vec<u8> in `CaptureRecord::payload`.** Plan uses
   `Vec<u8>`. The channel hot path already has the bytes on the
   heap (from `proto_bytes = cts.encode_to_vec()`), so cloning
   into the record is one heap copy. If profile shows this is a
   hotspot, switch to `bytes::Bytes` end-to-end.

## Design decisions worth pre-committing

- **Plaintext capture only.** `--capture-raw` deferred. Reduces
  attack surface (no AES key persistence) and matches the use case
  STEPS 18 / 19 actually need.
- **Single file, append-only, no rotation.** One `start`
  invocation = one file.
- **`record()` is sync, non-blocking.** Hot-path safety is the
...
- **Sync `CaptureReader` Iterator.** Replay is read-once, iterate;
  async would complicate the common case for no benefit.
- **Capture is opt-in; default `None`.** Tests for capture-off
  channels stay unchanged from STEPs 10 / 11.
- **Refactor `process_inbound{,_packet}` to return `Vec<u8>`
  plaintext, not the parsed `ServerToClient`.** Recv loop calls
  prost decode itself; tap fires on the plaintext. This makes the
  tap insertion clean and (incidentally) is the right move for the
  STEP 20 §20.2 "shared inbound-decode helper" parking-lot item if
  we ever take it.
...
- **STEP 08.** No change needed today — STEP 08's known-answer
  ...
  serve their purpose. If `--capture-raw` ever lands, STEP 08
  could regenerate vectors from real Zwift traffic for the
...
- **STEP 19 (compatibility tests).** Same — `compat/fixtures/*.cap`
  are ranchero-generated, no JS dep. Update the as-built doc to
  reflect the new fixture-generation recipe (e.g. "run `ranchero
  start --capture compat/fixtures/<name>.cap` for ~30 s during a
  live ride").
...
- `ranchero` (root crate):
  ...
  - `src/daemon/start.rs` (or wherever the `start` flow lives):
