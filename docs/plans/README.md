...
|  12 | ☐ | GameMonitor orchestration — sustainable end-to-end connectivity: auth + relay session + TCP + UDP + 1 Hz heartbeat + `udpConfigVOD` pool routing + idle suspension + watched-athlete switching + capture and tracing log | [STEP-12-game-monitor.md](STEP-12-game-monitor.md) |
| 12.1 | ☐ | TCP-only end-to-end smoke (first sub-step of STEP-12); auth + relay session + a single TCP channel + capture + tracing log; bounded by server-side timeout until the rest of STEP-12 adds UDP heartbeats | [STEP-12.1-tcp-end-to-end-smoke.md](STEP-12.1-tcp-end-to-end-smoke.md) |
| 12.2 | ☐ | `ranchero follow <file>` command for live capture-file tailing; reads a wire-capture file as it is written and prints each record (optionally decoded) to stdout; pairs with 12.1's `--capture` to validate the live stream from a second terminal | [STEP-12.2-follow-command.md](STEP-12.2-follow-command.md) |
