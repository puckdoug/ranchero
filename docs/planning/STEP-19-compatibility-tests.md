...
- [x] **19.5-T (harness)** — Add `tests/compat_metric_parity.rs` that
...
- [x] **19.5-I (capture generator)** — A helper that builds a `.bin` from a
  ...
  the generated `.bin`.
- [x] **19.5-I (pipeline)** — In the test, read frames via `CaptureReader`,
...
- [x] **19.5-T (comparison)** — Reuse `assert_json_parity`; extend it (or add
...
- [x] **19.5 (synthetic parity fixtures)** — `constant_power` (300 ticks,
  250 W constant) and `ramp` (5 × 60 ticks, 100→300 W), each with a
  committed `*.metrics.json` oracle generated by `generate_compat_fixtures`.
  `constant_power` confirms `stats.power.avg = 250.0`.
- [x] **19.5 (real-ride regression)** — Run `recorded_ride.bin` through the
  same pipeline, freeze ranchero's output as `recorded_ride.golden.json` (`{}`
  — no PlayerState frames in the monitor-only capture, which is the expected
  result). Marked `#[ignore = "slow: real recorded ride (206 s of relay
  telemetry)"]`. Labeled as a regression golden, not a JavaScript oracle.
- [x] Confirm `cargo test --test compat_metric_parity` is green.
